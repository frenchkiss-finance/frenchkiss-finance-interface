import { CurrencyAmount, JSBI, Token, Trade } from '@frenchkiss-libs/sdk'
import React, { useCallback, useContext, useEffect, useMemo, useState } from 'react'
import { ArrowDown } from 'react-feather'
import { CardBody, ArrowDownIcon, Button, IconButton, Text } from '@frenchkiss-libs/uikit'
...
import GlossWarningModal from 'components/GlossWarningModal'
...
const Swap = () => {
  ...
  const [approval, approveCallback] = useApproveCallbackFromTrade(chainId ? Number(chainId) : 0, trade, allowedSlippage)
  ...
  // This will check to see if the user has selected Gloss or SafeMoon to either buy or sell.
  // If so, they will be alerted with a warning message.
  const checkForWarning = useCallback(
    (selected: string, purchaseType: string) => {
      if (['GLOSS', 'SAFEMOON'].includes(selected)) {
        setTransactionWarning({
  ...
  const handleOutputSelect = useCallback(
    (outputCurrency) => {
      ...
      if (outputCurrency.symbol === 'GLOSS') {
  ...
  return (
    <Container>
      ...
      />
      <GlossWarningModal
        isOpen={transactionWarning.selectedToken === 'GLOSS'}
